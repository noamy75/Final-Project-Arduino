...
app.layout = html.Div(children=[daq.Joystick(id='my-joystick', label="Zumo Joystick", angle=0, size=100),
                                ...
                                html.Div([dcc.Slider(80, 200, 20, value=80, id='Speed-slider', marks=None,
                                                     tooltip={"placement": "bottom", "always_visible": True}),
                                          html.Div(id='Speed-slider-output-container')]),
                                html.Div([dcc.Slider(0.2, 2.5, 0.1, value=0.2, id='Kp-slider', marks=None,
                                                     tooltip={"placement": "bottom", "always_visible": True}),
                                          html.Div(id='Kp-slider-output-container')]),
                                html.Div([dcc.Slider(0, 100, 1, value=0, id='Kd-slider', marks=None,
                                                     tooltip={"placement": "bottom", "always_visible": True}),
                                          html.Div(id='Kd-slider-output-container')]),
                                html.Div([dcc.Slider(0, 2, 0.1, value=0, id='Ki-slider', marks=None,
                                                     tooltip={"placement": "bottom", "always_visible": True}),
...
def computations_output(n):
    global average, sd, accuracy_rate, velocity, battery
    return 'Average = {}, SD = {}, Accuracy rate = {}%, Velocity = {} cm/s, battery = {} V'.format(average, sd, accuracy_rate, round(velocity, 2), battery)
...
def update_graph_scatter(n):
    ...
    return {'data': [data],
            'layout': go.Layout(xaxis=xaxis, yaxis=dict(range=[0, 4000]))}
...
def TransmitThread():
    ...
    while ser.isOpen:
        ...
        if sum(counter_clockwise_arr) > sum(clockwise_arr):
            ...
            direction = '1'
        msg = ''
        msg = str(auto_flag) + ';' + direction + '.' + str(joyX) + ',' + str(joyY) + '?' + str(Kp) + '!' + str(
            Kd) + '@' + str(Ki) + '#' + str(Speed) + '\r\n'
...
def ReceiveThread():
    global line_sensor_values, clockwise_arr, counter_clockwise_arr, counter, position, X, Y, average, sd, out_of_bounds_counter, accuracy_rate, DT, timestamp, velocity, battery
    while (ser.isOpen):
        if (ser.in_waiting > 0):
            ...
            if auto_flag == 1:
                ...
                sd = round(sqrt(float(sum([(position - average) ** 2 for position in Y[10:]])) / len(Y[10:])),
                           2) if len(Y) > 10 else 0
                DT = data["DT"]
